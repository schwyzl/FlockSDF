...
class GPUApp {
  ...
  constructor(){
    const canvas = document.querySelector("canvas#webgpuApp") as HTMLCanvasElement;
   
    this.setupWebGPU(canvas).then(()=>{this.init()});
  }
  ...
  private init(){
    ...
    new GameOfLife(this.device!, this.context!);
  }
}
